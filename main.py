# Modificar la sección de importaciones en main.py
from interface import *
from chords import ChordRecognitionThread  # Importación explícita de la clase
from key import KeyRecognitionThread
from tempo import TempoDetectionThread
# Importar tanto los temas originales como los pasteles
from theme import light_theme, dark_theme, pastel_light_theme, pastel_dark_theme
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.is_muted = False
        self.is_dark = True
        self.use_pastel = True  # Nueva variable para controlar el uso de temas pasteles
        self.load_stack = 1
        self.setAcceptDrops(True)
        
        # Aplicar tema inicial según configuración
        if self.is_dark:
            self.setStyleSheet(pastel_dark_theme if self.use_pastel else dark_theme)
        else:
            self.setStyleSheet(pastel_light_theme if self.use_pastel else light_theme)
    ...
    def toggle_theme(self):
        ...
        # Aplicar el tema correspondiente según las preferencias
        if self.is_dark:
            self.setStyleSheet(pastel_dark_theme if self.use_pastel else dark_theme)
        else:
            self.setStyleSheet(pastel_light_theme if self.use_pastel else light_theme)
    ...
    def load_audio(self, fileName=None):
        if not fileName:
            ...
            fileName, _ = QFileDialog.getOpenFileName(self, "Abrir Archivo de Audio", "", "Archivos de Audio (*.wav *.mp3 *.m4a *.aac)", options=options)
        if fileName:
            self.timer.stop()
            self.player.stop()
    ...
    def on_key_recognized(self, key):
        ...
        current_text = self.ui.keyLabel.text()
        if current_text:
            updated_text = f"{key}  |  {current_text}"
            self.ui.keyLabel.setText(updated_text)
        else: 
    ...
    def export_chords(self):
        if self.chords:
            ...
            with open(file_path, 'w') as file:
                file.write(f"Tonalidad: {getattr(self, 'key', 'Desconocida')}\n")
                file.write(f"Tempo: {getattr(self, 'tempo', 'Desconocido')} BPM\n\n")
                file.write("Acordes:\n")
                for chord in self.chords:
                    start_time, end_time, chord_label = chord
                    file.write(f"({self.format_time(start_time)} - {self.format_time(end_time)}): {chord_label}\n")
            
            print(f"Acordes exportados a: {file_path}")
    ...
    def keyPressEvent(self, event):
        ...
        # Añadimos una nueva tecla para cambiar entre temas pasteles y tradicionales
        if event.key() == Qt.Key_S:
            self.toggle_pastel_theme()
    ...
    def toggle_pastel_theme(self):
        """Cambia entre el tema pastel y el tema tradicional"""
        self.use_pastel = not self.use_pastel
        # Aplicar el tema actual pero en la variante correspondiente
        if self.is_dark:
            self.setStyleSheet(pastel_dark_theme if self.use_pastel else dark_theme)
        else:
            self.setStyleSheet(pastel_light_theme if self.use_pastel else light_theme)
        
        print(f"Usando tema pastel: {'Sí' if self.use_pastel else 'No'}")
    
    def closeEvent(self, event):
        """Se ejecuta cuando se cierra la ventana"""
        # Limpiar los archivos de caché creados en esta sesión
        try:
            ChordRecognitionThread.clear_chord_cache(all_files=False)
        except:
            pass
        event.accept()
